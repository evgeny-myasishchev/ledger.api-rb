...
module JwtHelpers
  ...
  def stub_jwks_endpoint(cert = create_x509_cert)
    response = {
      keys: [
        { kid: "kid1-#{FFaker::Lorem.word}", x5c: [x509_to_pkix(cert)] },
        { kid: "kid2-#{FFaker::Lorem.word}", x5c: [x509_to_pkix(create_x509_cert)] },
        { kid: "kid3-#{FFaker::Lorem.word}", x5c: [x509_to_pkix(create_x509_cert)] }
      ]
    }
