...
RSpec.describe 'V2::Accounts', type: :request do
  include JsonApiSerialize
  include AuthHelpers
  include MatchingLedgerHelpers

  describe 'GET /v2/ledgers/:ledger_id/accounts' do
    it_behaves_like 'authorized action', :get,
                    path: { helper: :v2_ledger_accounts_path, ledger_id: 'fake' },
    ...
    it 'should return accounts' do
      ...
      get v2_ledger_accounts_path(ledger_id: ledger_user.ledger_id), with_valid_auth_header(scope: 'read:accounts', sub: ledger_user.user_id)
    ...
    it 'should fail with 404 if ledger is not shared with user' do
      ...
      get v2_ledger_accounts_path(ledger_id: ledger.id), with_valid_auth_header(scope: 'read:accounts', sub: ledger_user.user_id)
      should_fail_with_ledger_not_found(response, ledger)
    end
  end

  describe 'POST /v2/ledgers/:ledger_id/accounts' do
    ...
    it 'should fail with 404 if ledger is not shared with user' do
      ...
      post v2_ledger_accounts_path(ledger_id: account.ledger_id),
    ...
    it 'should create a new account' do
      ...
      json = json_api_serialize(account).as_json.with_indifferent_access
      post v2_ledger_accounts_path(ledger_id: ledger_user.ledger_id),
           with_valid_auth_header(scope: 'write:accounts', sub: ledger_user.user_id).merge(params: json)
